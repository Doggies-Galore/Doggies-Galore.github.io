...
# Data buffers
devices_buffer = {}  # {MAC: (RSSI, Channel, Timestamp, Interface, Router MAC)}
networks_buffer = {}  # {SSID: {BSSID, signal_strength, noise, channel, timestamp, interface}}
    ...
    with open(network_csv, 'a', newline='') as net_file, open(device_csv, 'a', newline='') as dev_file:
        ...
        for ssid, info in networks_buffer.items():
            network_writer.writerow([
                ssid, info['BSSID'], info['signal_strength'], info.get('noise', 'N/A'),
        ...
        for mac, data in devices_buffer.items():
            device_writer.writerow([
                mac, data[0], data[1], data[2], data[3], data[4]
            ])
...
def display_data():
    ...
    for mac, data in devices_buffer.items():
        router = data[4] if data[4] else "Unknown"
        print(f"MAC: {mac} | RSSI: {data[0]} dBm | Channel: {data[1]} | Interface: {data[3]} | Router: {router}")

    print("\nNearby Networks:")
    for ssid, info in networks_buffer.items():
        print(f"SSID: {ssid} | BSSID: {info['BSSID']} | RSSI: {info['signal_strength']} dBm | "
              f"Noise: {info.get('noise', 'N/A')} | Channel: {info['channel']}")

# Packet handler
def packet_handler(packet, interface, channel):
    if packet.haslayer(Dot11):
        mac = packet.addr2  # Device MAC address
        router_mac = packet.addr3  # Router MAC (BSSID)
        if mac:
            # RSSI extraction
            ...
            devices_buffer[mac] = (rssi, channel, timestamp, interface, router_mac)
        ...
        if packet.type == 0 and packet.subtype in (8, 5):  # Beacon/Probe Response
            ...
            bssid = packet.addr2  # BSSID of the network

            # Store network info in buffer
            networks_buffer[ssid] = {
                'BSSID': bssid, 'signal_strength': rssi, 'noise': noise,
...
if __name__ == "__main__":
    ...
    with open(network_csv, 'w', newline='') as net_file:
        network_writer = csv.writer(net_file)
        network_writer.writerow(["SSID", "BSSID", "Signal Strength (dBm)", "Noise (dBm)", 
                                 "Channel", "Timestamp", "Interface"])

    with open(device_csv, 'w', newline='') as dev_file:
        device_writer = csv.writer(dev_file)
        device_writer.writerow(["MAC", "Signal Strength (dBm)", "Channel", 
                                "Timestamp", "Interface", "Router MAC"])

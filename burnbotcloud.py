...
def cloud():
    ...
    def slow_print(text, delay=random.randint(1,3)):
        time.sleep(delay)
        print(text)

    slow_print("Burnbot v1.5 Cloud at your service!")
    ...
    #On the other hand, if they get all gramatical... take care of them too. 
    #omg these sound like death threats :sob:
    name=name.lower()
    name=name.split("'")[0]
    if "name is" in name.lower():
        name = name.split("name is")[1].strip()
    elif "i am" in name.lower():
        name = name.split("i am")[1].strip()
    elif "im" in name.lower():
        name = name.split("i'm")[1].strip()
    name = name.capitalize()
    if name != name.split(" ")[0]:
        slow_print("Ok damn- no need to get so formal. Next can you tell me your SSN?")
        slow_print("Anyway...")
    name = name.split(" ")[0]
    age = input(f"Nice to meet you, {name}. How old are you? ")
    #Run filter to check if alphaneumeric characters are present (and check if it's empty!)
    #Convert age to integer if possible, otherwise set to 0 for roasting

    if age == "":
        slow_print("What- I'm not gonna steal your personal information. I'm just a bot (>v<).")
        slow_print("If that's how it's gonna be then... goodbye!")
        exit()
    if not age.isdigit():
        #Get just the number
    ...
    if age < 1 or age > 120:
        slow_print("That's not a real age. You must be a bot. I don't talk to clankers. Goodbye.")
        exit()
    if age < 14:
        #Blah blah blah
    ...
    elif "i don't" in work.lower() or "t have a job" in work.lower() or "don't have a job" in work.lower() or "no job" in work.lower():
        slow_print("You're unemployed? Damn, good luck with that. T_T")
    else:
    ...
    def bullying():
        ...
        while True:
            ...
            if "hey" in bullying.lower() or "hi" in bullying.lower() or "hello" in bullying.lower():
                ...
                continue
            elif "what's up" in bullying.lower() or "sup" in bullying.lower() or "s up" in bullying.lower():
    ...
    def rapidfire():
        ...
        app_roasts = []
        app_checks = [
            ("discord", "Let me guess - your social life is just different shades of memes and voice chat awkwardness? Discord really brings out the best in us, doesn't it?"),
            ("steam", "Your game library looks like a graveyard of abandoned dreams and unfulfilled potential. Maybe one day you'll actually play all those Steam sale purchases."),
            ("minecraft", "Still punching trees in 2025 while your friends have moved on to actual careers? At least your virtual house looks nice."),
            ("fortnite", "Your dance moves in-game probably mirror your real-life social awkwardness. Just embrace the cringe at this point."),
            ("roblox", "Blocky avatars and broken physics - because who needs realistic graphics when you can have... whatever this is supposed to be?"),
            ("spotify", "Your most played playlist is called 'existential crisis mix' with a side of 'songs to cry to', isn't it? Don't worry, we've all been there."),
            ("zoom", "Ah, the digital equivalent of staring into the void while pretending to care about Karen from accounting's cat. Modern work is truly magical."),
            ("teams", "The sweet symphony of 'is your mic on?' and 'you're on mute' - Microsoft really outdid themselves with this one."),
            ("slack", "Because nothing says 'work-life balance' like being reachable 24/7. Your boss must be so proud."),
            ("chrome", "How does it feel knowing your browser uses more RAM than your first computer had storage? Modern problems require modern solutions, I guess."),
            ("firefox", "You're that person who still uses a flip phone but calls it 'retro', aren't you? At least you're not using Netscape... yet."),
            ("edge", "Oh honey, did Microsoft hold your family hostage or do you just enjoy pain? There are better browsers, you know."),
            ("safari", "Customization is overrated anyway, right? Who needs extensions when you can have... whatever this is supposed to be?"),
            ("notepad++", "Ah, the text editor for people who think 'modern IDEs are too complicated' while having 47 tabs open for simple tasks. Classic Notepad++ user behavior."),
            ("vscode", "How many extensions does it take before you admit you have a problem? Asking for a friend who definitely doesn't have VSCode open with 30+ extensions."),
            ("pycharm", "The overpriced calculator for people who think their Python scripts are NASA-level code. But hey, at least the debugger is nice."),
            ("intellij", "Let me guess - you write 'Hello World' and the IDE creates 17 configuration files and a Maven wrapper. Classic IntelliJ overkill."),
            ("word", "The digital equivalent of using a sledgehammer to hang a picture frame. Microsoft Word: because sometimes you just need a bulldozer to plant a flower."),
            ("excel", "Ah yes, the world's most overqualified calculator that you've somehow turned into a database, project manager, and your personal cry for help. Pivot tables won't fill the void, Karen."),
            ("powerpoint", "Nothing says 'I value your time' like 47 slides of corporate buzzwords and ClipArt from 2003. Your coworkers must love you."),
            ("obsidian", "You've spent more time organizing your notes than actually doing anything with them, haven't you? Those graphs and connections aren't going to make themselves."),
            ("obs", "So you're either a streamer with 3 viewers or someone who records hours of footage they'll never edit. Either way, I'm concerned about your hard drive space."),
            ("tiktok", "Your attention span is now shorter than the time it takes to boil water for instant noodles. Maybe try reading a book? Just a thought."),
            ("snapchat", "Ah yes, the app where your memories disappear faster than your motivation on a Monday morning. At least your cringey selfies won't come back to haunt you."),
            ("instagram", "Hours spent curating the perfect illusion that your life is more interesting than it actually is. That sunset pic with the deep quote? We all know you just took it in your backyard."),
            ("facebook", "The digital equivalent of your weird uncle's conspiracy theories mixed with baby photos and political rants. It's like a family reunion, but somehow more awkward."),
            ("whatsapp", "Nothing says 'family bonding' like being added to a group chat you never wanted to be in. Thanks for the 200 unread messages about Aunt Carol's cat, I guess?"),
            ("telegram", "Using a messaging app doesn't make you a cybersecurity expert, but tell yourself whatever helps you sleep at night, Mr. 'I only use end-to-end encryption for my grocery lists'."),
            ("git", "Your commit history is probably just 'fix' and 'oops' repeated 47 times, isn't it? Don't worry, we've all been there... though maybe not 47 times in one project."),
            ("docker", "Couldn't get your code to run locally, so you put it in a container and called it 'devops'. Modern problems require over-engineered solutions, right?"),
            ("vim", "How many hours of your life have you spent trying to exit this editor? Don't worry, we won't judge... much. It's only been 15 minutes of frantic Googling so far."),
            ("emacs", "The operating system that's missing a good text editor but comes with a built-in existential crisis. At least you can write emails in it, or something."),
            ("blender", "Your hard drive is just full of half-finished donut tutorials and abandoned character models, isn't it? One day you'll finish that animation... one day."),
            ("gimp", "Free software is great until you spend three hours trying to figure out which of the 47 tools does what you want. At least you saved $20 on Photoshop, right?"),
            ("photoshop", "Reality is just a suggestion when you've got the clone stamp tool. Those vacation photos with you on Mars look totally legit, by the way."),
            ("premiere", "Spending more time rendering than actually editing? Your film school professor would be so proud of your 5-minute YouTube video that took 15 hours to make."),
            ("unity", "Another game jam project that's 90% done but will never see the light of day, huh? At least the asset store looks pretty."),
            ("unreal", "Your PC sounds like it's about to take off every time you hit 'Play'. Maybe tone down those 8K textures for your 'Flappy Bird' clone?"),
            ("android studio", "Watching that gradle build progress bar is more exciting than your social life. And it never gets past 87%, does it?"),
            ("xcode", "Simulator not responding' is basically Apple's way of saying 'go get a coffee... or three'."),
            ("calculator", "Basic arithmetic shouldn't require an app, but here we are. Don't worry, we won't tell anyone you needed help with 7 x 8."),
            ("calendar", "Color-coding your existential dread into neat little time blocks doesn't make it any less depressing, but at least it looks organized."),
            ("onenote", "Your notes are like a treasure map where X marks the spot, but the map is in 17 different notebooks and the ink is smudged."),
        ...
        found_apps = [(app, roast) for app, roast in app_checks if any(app in a for a in user_apps)]
        if found_apps:
            slow_print("\nApp scan complete. Let's see what your installed apps say about you...\n")
            time.sleep(1)
            for app, roast in found_apps:
                # Format app name (preserve acronyms like VLC, MATLAB, etc.)
                app_name = app.upper() if app.upper() == app or len(app) <= 3 else app.capitalize()
                
                # Natural language intros with more variety and sass
                intro = random.choice([
                    f"I see you have {app_name}...",
                    f"Ah, {app_name} is installed...",
                    f"Found {app_name} on your system...",
                    f"Let's talk about {app_name}...",
                    f"I notice {app_name} in your apps...",
                    f"Oh look, {app_name}...",
                    f"Well well well, what do we have here? {app_name}...",
                    f"Let me guess, you think you need {app_name}...",
                    f"I found {app_name} lurking in your system...",
                    f"Hmm, {app_name}... interesting choice...",
                    f"I was going to roast {app_name}, but it's already a joke...",
                    f"Let me put on my judgey glasses... ah yes, {app_name}...",
                    f"*checks notes* Ah yes, {app_name}...",
                    f"I see you're a person of culture with {app_name}...",
                    f"Oh dear, {app_name}? Really?",
                    f"Let me put on my best disappointed face... {app_name}, really?",
                    f"I wasn't going to judge, but then I saw {app_name}..."
                ])
                
                # Format the roast to start with capital letter
                formatted_roast = roast[0].upper() + roast[1:]
                
                # Print with natural pacing
                slow_print(f"\n{intro} {formatted_roast}")
